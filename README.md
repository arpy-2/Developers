# Developers
### 1. [Google for Developers](https://developers.google.com/?hl=es)
<p>Google's platform for developing web and mobile apps that helps developers build apps and games</p>

### 2. [Microsoft Developer](https://developer.microsoft.com/es-es/)
<p>Developer communities, tools, and resources to solve today's problems and build tomorrow's innovations</p>

### 3. [Apple Developer](https://developer.apple.com/)
<p>Join the Apple Developer Program to reach customers around the world on the App Store for iPhone, iPad, Mac, Apple Watch, Apple TV, and Apple Vision Pro</p>
